...
def _make_base_tools(workspace: str, chrome_cdp_port: int = 9222) -> ToolRegistry:
    ...
    tools = ToolRegistry()
    sf_tool = SendFileTool(workspace=workspace)
    chrome_tool = ChromeBrowserTool(workspace=workspace)
    chrome_tool.set_send_file(sf_tool)
    chart_tool = BinanceChartTool(workspace=workspace)
    chart_tool.set_send_file(sf_tool)

    tools.register(WebSearchTool())
    tools.register(WebFetchTool())
    tools.register(ExecCmdTool(workspace=workspace))
    tools.register(chrome_tool)
    ...
    tools.register(sf_tool)
    tools.register(chart_tool)
